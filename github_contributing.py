...
def update_fork():
    ...
    subprocess.run(
        ["git", "merge", "upstream/main"], check=True
...
def push_changes(branch_name, commit_message):
    ...
    )  # Switch to the branch where your changes are
    ...
    subprocess.run(
        ["git", "push", "fork", branch_name], check=True
    )  # Push the commit to the same branch in your fork
...
def create_pull_request(branch_name, pr_title, pr_file):
    ...
    subprocess.run(
        ...
        check=True,
    )  # Create a pull request with the specified title and markdown body
...
def main():
    ...
    parser_create_branch.add_argument(
        "--branch-name", required=True, help="The name for the new branch"
    )

    # Subparser for pushing changes
    parser_push = subparsers.add_parser(
        "push-changes", help="Push local changes to the fork"
    )
    parser_push.add_argument(
        "--branch-name", required=True, help="The name of the branch you are working on"
    )
    parser_push.add_argument(
        "--commit-message", required=True, help="The commit message for your changes"
    )

    # Subparser for creating a pull request
    parser_pr = subparsers.add_parser(
        "create-pr", help="Create a pull request to the original repository"
    )
    parser_pr.add_argument(
        "--branch-name",
        required=True,
        help="The name of the branch the pull request is from",
    )
    parser_pr.add_argument(
        "--pr-title", required=True, help="The title of your pull request"
    )
    parser_pr.add_argument(
        "--pr-file",
        required=True,
        help="The markdown file path for your pull request description",
    )
